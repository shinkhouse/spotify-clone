...
import { GenericState } from '../models/generic-state.model';
import { SpotifyTrackExtended } from '../models/track-extended.model';
...
export class PlaybackStore extends ComponentStore<PlaybackState> {
    readonly playback$ = this.select((s) => s.data).pipe(filter((data) => !!data)) as Observable<Spotify.PlaybackState>;
    ...
    readonly currentTrack$: Observable<SpotifyTrackExtended | null> = this.playback$.pipe(
        ...
        map(({ context, track_window }) => {
            ...
            const getPlaylistUrl = (uri: string | null) => {
                ...
                return isPlaylist ? `/playlist/${this.getIdFromUri(uri)}` : '';
            };
            const albumId = this.getIdFromUri(album.uri);
            ...
            const trackExtended: SpotifyTrackExtended = {
                ...
                artists: track.artists.map((artist) => {
                    const artistId = this.getIdFromUri(artist.uri);
    ...
    constructor(private trackApi: TrackService) {
